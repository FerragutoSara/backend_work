...
1. avviare l’applicazione con `uvicorn app.main:app --reload`
2. aprire `http://127.0.0.1:8000/docs`
3. eseguire il POST `/users/`
4. copiare l’id restituito
5. eseguire il GET `/users/{user_id}`
6. eseguire il PUT `/users/{user_id}`
7. eseguire il DELETE `/users/{user_id}`
8. verificare in Compass le variazioni della collection
